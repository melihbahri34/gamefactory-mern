...
function App() {
	...
	async function registerUser(event) {
		...
		const response = await fetch('http://localhost:2002/api/register', {
			method: 'POST',
			headers: {
	...
	return (
		<div class="pt-16">
          <div class="w-full max-w-sm p-6 m-auto bg-white rounded-md shadow-md dark:bg-gray-800">
            <h1 class="text-3xl font-semibold text-center text-gray-700 dark:text-white">Try for Free</h1>
    
            <form class="mt-6" onSubmit={registerUser}>

                <div>
                    <label for="name" class="block text-sm text-gray-800 dark:text-gray-200">Full Name</label>
                    <input type="text"
            ...
            <div class="flex items-center justify-between mt-4">
                ...
                <a href="#" class="text-xs text-center text-gray-500 uppercase dark:text-gray-400 hover:underline">or register with Social Media</a>
            ...
            <p class="mt-8 text-xs font-light text-center text-gray-400"> Do you have an account? <a href={ROUTES.LOGIN}
